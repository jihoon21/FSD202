...
$(function () { ////// jQB ///////////////////////
    ...
    이런 결과로 볼때 스크롤방향과 매칭해보면
        음수가 나오면 아래쪽방향 스와이프 -> 스크롤을 위로
        양수가 나오면 윗쪽방향 스와이프 -> 스크롤을 아래로
    */
    ...
    $(document).on("touchend",function(e){//e-이벤트전달변수
        ...
        //양수: 윗쪽방향 스와이프 -> 스크롤을 아래로 -> 페이지번호증가
        if (delta > 0) { 
            pno++;
            if (pno === totnum) pno = totnum - 1; //끝번호고정!
        } /////////// if //////////////////
        
        //음수: 아래쪽방향 스와이프 -> 스크롤을 위로 -> 페이지번호감소
        else { 
            pno--;
