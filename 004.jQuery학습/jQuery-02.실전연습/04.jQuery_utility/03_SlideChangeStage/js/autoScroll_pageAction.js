...
$(function () { ////// jQB ///////////////////////
    ...
    init = function () {
        ...
        $("#pg2 .minfo").hide();//display:none만들기!
        ...
        // 4. 봉이김선달 : 중앙에서 스케일을 0으로 함(원래크기로 변경예정)
        // 주의사항 : transform은 animate로 할 수 없다!
        // 이 경우에는 transition으로 애니메이션 적용한다!
        // 유의사항: 이미 transform으로 translate(-50%,-50%)가 적용되었으므로 scale(0)에 같이 써줘야함!
        $("#pg4 .minfo").css({
            transform: "translate(-50%,-50%) scale(0)"
        }); ////// css ////////
        
        // 5. 비밀은 없다 : y축으로 180도 뒤집어 놓고 투명하게 설정
        ...
        // 6. 아가씨
            ...
            $("#pg6 .minfo").css({
               transform: "translate(-50%,-50%) scale(1) rotate(0deg)",
                transition: "all 1s cubic-bezier(0.76, 0.21, 0.01, 0.83) 0s"
            });///// css /////
        }//// else if //////
        // 7. 탐정홍길동
        else if(pno===6){
            ...
            $("#pg7 .minfo").css({
                ...
                opacity: 1,
                transition: "all 2s cubic-bezier(0.57, 0.01, 0, 0.92) 0s"
            });////// css ///////
        }////// else if ///////
    ...
    1. 기능: 마우스휠을 위나 아래로 작동 시킬때 페이지가 위나 아래로 자동으로 애니메이션 되는 기능
    
    2. 이벤트: 마우스휠을 움직일때 발생하는 이벤트는?
        - mousewheel (오리지널 이벤트)
        - wheel (신규이벤트-벤더사의 웹표준이 아직아님!)
        - DOMMouseScroll - (파이어폭스 전용 휠이벤트)
        -> 결론적으로 모두 한꺼번에 지원하는 이벤트가 없으므로 mousewheel + DOMMouseScroll을 같이 사용할 것임!
    ...
    4. 마우스 휠 이벤트 대상: 보통 document에 적용함
        -> 유의 사항: document, window, body 3가지 중요객체에서 기본기능 중단을 못하도록 코드가 변경되었음(2019년경) 이것을 대상으로 해서 e.preventDefault() 같은 코딩하면 에러가 나면서 작동도 불가함!(만약 사용하려면 body내부에 body를 대신하는 요소를 따로 만들어서 사용해야한다!)
    */
    ...
    $(document).on("mousewheel DOMMouseScroll", function (e) {
        ...
        // document, window, body 객체는 막을 수 없다!
        ...
        // return false 하면 에러남! 왜? document니까! 법이바뀜!!!
        // return true 해야함! 돌아가되 기능은 켜놓고 가(스크롤되게해!!)
        ...
        // 1. 마우스휠 방향 알아내기
        e = window.event || e;
        // 이벤트 전달값이 window 오리지널 이벤트가 사용가능하면 사용되도록 보완코드를 작성함!
        // 변수 = 경우1 || 경우2;
        // 변수에 경우1과 경우2 중 true(유효한것)인것이 할당됨
        /*
        ...
        */
        ...
        /*
            ...
            3. 대소문자 구분없이 찾을때 i라는 플래그문자를 사용함
            예) /,/g -> 모든 콤마를 찾아라!
                /Firefox/i
        ...
        */
        ...
        // 2. 마우스휠 방향에 따라 페이지번호 증감!
        if (delta < 0) { // -147 아랫방향 스크롤(다음페이지)
            pno++;
            if (pno === totnum) pno = totnum - 1; //끝번호고정!
        } /////////// if //////////////////
        else { // 147 윗방향 스크롤(이전페이지)
            pno--;
        ...
        $("html,body").stop().animate({
            scrollTop: pgpos + "px"
        }, 1200, "easeOutExpo",function(){// 페이지 이동후 페이지 액션!
            pageAction();
        ...
        // 블릿네비게이션 클리새 넣기
        $(".bnav li").eq(pno).addClass("on").siblings().removeClass("on");
