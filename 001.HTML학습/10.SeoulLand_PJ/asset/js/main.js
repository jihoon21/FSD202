...
$(function () { ///// jQB //////////////////////////////
    ...
    $(".tabTit a").click(function (e) {
        ...
        // 왜 필요한가? 탭 컨텐츠때문에 같은 순서를 셋팅하기위해!
        let idx = $(this).parent().index();
        // $(this)는 a요소 이므로 부모(parent)로 올라간다
        console.log("순번:" + idx);

        // 2. 탭메뉴는 클릭된 자신(this)의 부모(li)에게 class="on"
        // 다른형제 요소들(li들)은 class="on"을 지운다!
        $(this).parent().addClass("on").siblings().removeClass("on");

        // 3. 탭컨텐츠는 탭메뉴의 순번과 동일한 순번의 li에 class="on"
        // 다른 형제 요소들(li들)은 class="on"을 지운다!
        $(".tabCont li").eq(idx).addClass("on").siblings().removeClass("on");
    ...
    $(".upb").click(function (e) {
        ...
        // 1. 먼저 맨뒤 li를 맨앞으로 이동과 동시에 top값을 -92px 즉, li하나가 위에 나가있는 위치로 변경!
    ...
    // 문제가있다!!? 전역변수 구역에 이미 같은이름의 autoSlide(), autoT, clearAuto(), autoI 가 있다
    // 기존방식은 단순히 이름을 바꿔줘야했다!
    // 제이쿼리 영역안에 있으므로 이 영역에서만 유효한 같은이름의 변수로 선언할 수 있다! 그 방법은 var 대신 let을 쓰는 것이다!
    /////////////////////////////////////////////////////
    ...
    기능정의: 이동버튼을 클릭하면 배너li를 변경한다.
            ...
            5. 시나리오:
                ...
                (5) 전역변수에 현재 슬라이드 li순번을 기록하고
                    이것을 업데이트하여 변경된 순번의 요소에 class를 "on"값으로 넣어준다.
